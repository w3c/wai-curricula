...
* code instant feedback, such as that results from validating input from the user as it is being provided, using WAI-ARIA `aria-live` regions with the appropriate interruption level, so that they are communicated to assistive technologies
