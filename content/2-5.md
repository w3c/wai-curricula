...
* Demonstrate use of a screen reader to complete a set of form elements. Emphasize that screen reader users may only detect information present in the form labels as they are navigating in focus mode. Provide examples of dedicated screen reader keystrokes that they can use to navigate to specific types of form controls, such as e for edit boxes, x for check boxes, or a for radio buttons.
...
* Refer to common misunderstandings when completing form elements. For example, which format a date should have or how a phone number should be written. Explain that consistent identification of the fields and instructions help all users and are essential for some such as those with cognitive disabilities.
...
* Introduce explicit associations by means of the `for` attribute. Explain that its value must be a unique id reference of the form control that the label pertains to.
...
* Introduce additional means to provide labels when visible text is not convenient. For example, the `title` element, the `aria-label`, or `aria-labelledby` attributes. Mention that the `title` element may not be well supported by some assistive technologies. Emphasize that contents provided using WAI-Aria are just available for assistive technologies and are not visible on screen.
...
Present several HTML5 input types and additional techniques that help users enter information in the expected type an format.
...
* Indicate that the `required` element denotes that a form field element cannot be left empty. Comment that it is well supported by assistive technologies. Emphasize that it is also good practice to include a textual indicator in the label itself, such as the word "required" or an asterisk (*), so that all users are aware.
* Present some common HTML5 elements that help validation and allow users to enter data more reliably.
* Explain that the `pattern` attribute serves to validate common ways of entering information, such as car license plate numbers, phone numbers, or dates.

#### Ideas to Assess Knowledge

Optional ideas to support assessment.

* Practice &mdash; Students are presented with a set of required form fields and are asked to programmatically convey that fact. Assess how students make use of the `required` element and/or some text in the label of the form control.
* Practice &mdash; Students are presented with a set of edit boxes and check boxes. Students are asked to choose an HTML5 input type that would stand for the specific purpose of that field. Assess students knowledge of HTML5 form input types.
* Practice &mdash; Students are presented with a form field that collects phome numbers of a given country. Students are asked to provide a reliable mechanism to ease validation. Assess how students use the `pattern` attoibute together with regular expressions to indicade the expected input format.
...
* Mention that time limits in forms should be avoided to allow users to complete them at their own pace. When there is a need to establish the limit due to security reasons, users have to be notified of that in advance and be provided with a way to extend or stop the limit. If the time limit is essential to a valid submision, then there is no need to allow users to extend or stop the limit.
* Introduce the different ways in which errors can be announced after form submission. Explain that a warning message should be included in the title of the page or its main heading, so that all users are aware that there submissions have produced errors.
* Explain that it is good practice to use lists and list items to group error messages. Add that they could link to the form field they pertain to by using anchors.
...
Optionalideas to support assessment.
...
* Indicate the presence of these errors in the main heading or title of the sage.
* Identify each erroneous field in a list of ellements.
* Linkeach list item with its corresponding form field.
