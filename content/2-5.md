...
* Practical &mdash; Students are asked to activate a set of form controls by clicking their labels. Assess how students understand the relevance of proper click targets for users with mobility impairments.
...
* mark up additional instructions that are outside of the form controls' label using the HTML attribute `title` or `placeholder`, or the WAI-ARIA attributes `aria-labelledby` or `aria-describedby`
