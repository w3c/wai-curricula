...
* Label controls so that they can be identify by users and assistive technologies.
...
* Understand how keyboard users navigate through form fields using the tab key or specific keystrokes.
* Understand that screen reader users only perceive information attached to focusable elements as they navigate using forms mode.
* Understand that users with low vision or with mobility impairments need larger targets to interact with form fields.
* Understand that users with cognitive disabilities rely on consistent identification and instructions to understand form fields' purpose.
