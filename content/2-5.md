...
* Label controls appropriately so that they can be identified by users and assistive technologies.
...
* Refer to common misunderstandings when completing form elements. For example, in which format a date should be entered or how a phone number should be written. Explain that these instructions help all users and are essential for users with cognitive disabilities.
...
* Practice &mdash; Students are presented with a set of edit boxes and check boxes. Students are asked to choose an HTML5 input type that would serve the specific purpose of each of the fields. Assess students knowledge of HTML5 form input types.
* Practice &mdash; Students are presented with a form field that collects phone numbers of a given country. Students are asked to provide a reliable mechanism to ease validation. Assess how students use the `pattern` attribute together with regular expressions to indicate the expected input format.
...
* Mention that time limits in forms should be avoided to allow users to complete them at their own pace. Explain that when there is a need to establish the limit due to security reasons, users have to be notified in advance and be provided with a way to extend or stop the limit. If the time limit is essential to a valid submision, then there is no need to allow users to extend or stop the limit.
