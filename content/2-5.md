...
* write markup for groups of form controls that share a common purpose and provide labels for such groups
* write markup for instructions, notifications, and other feedback so that they are perceivable by all users
...
* write markup for labels for form controls using the HTML element `label` and attribute `for`
* identify situations in which it is necessary to have labels for form controls appear only on hover or focus but still have them permanently available for assistive technologies and mark up such labels using the following techniques as appropriate:
  ...
  * HTML element `title`
* write markup for groups of form controls using the HTML element `fieldset` and label such groups of form controls using the HTML element `legend`
...
* write markup for overall instructions for forms and specific instructions for individual form controls and groups of form controls in such a way that is presented clearly to all users, including through assistive technologies in *forms mode*
* write markup for required form controls using label text in addition to the HTML attribute `required` or the WAI-ARIA attribute `aria-required`
...
* write markup for instant feedback, such as that resulting from validating input from the user as it is being provided, using WAI-ARIA `aria-live` regions, and identify the appropriate priority values `polite`, `assertive`, and `off` for feedback
* write markup for alerts using WAI-ARIA `role="alert"` and identify situations in which it is appropriate to interrupt users
