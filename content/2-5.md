...
* explain how labels and instructions allow people with disabilities to navigate and fill in form controls
...
* explain how labels allow people with disabilities to identify and fill in form controls
...
* Provide examples of explicit and implicit associations using the `label` element. Explain that an implicit association can be achieved by wrapping the `label` element and its content around the form control. Explain explicit associations by means of the `for` attribute and indicate that its value must be a unique id reference of the form control that the label refers to. Explain use of other explicit associations using the WAI-ARIA attributes `aria-labeled` or `aria-labelledby`. Examples on how to label form controls are provided in the WAI tutorials, [Labeling controls](https://www.w3.org/WAI/tutorials/forms/labels/).
...
Explain how to code mechanisms for users to extend or stop time limits.
...
* Practice &mdash; Students are presented with a form with a time limit and are asked to provide one mechanism to stop the limit and another one to extend it. Assess students' knowledge of how to code mechanisms to extend stop time limits.
