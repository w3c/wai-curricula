...
* Refer to common misunderstandings when completing form elements such as which format a date should have or how a phone number should be written. Explain that consistent identification of the fields and instructions help all users and are essential for some such as those with cognitive disabilities.
...
* Use the `fieldset` and `legend` elements to group together several form fields that share the same purpose.
...
* Introduce additional means to provide labels, such as the `title` element, the `aria-label`, or `aria-labelledby` attributes. Mention that the `title` element may not be well supported by some assistive technologies. Emphasize that contents provided using WAI-Aria are just available for assistive technologies and are not visible on screen.
...
* Explain that the `pattern` element serves to validate common ways of entering information, such as car license plate numbers, phone numbers, or dates.
...
* Introduce the different ways in which errors can be announced after form submission. Explain that a message like WErrors have been detected" should be included in the title of the page or its main heading, so that all users are aware.
* Explain that it is good practice to use lists and list items to group error messages. Add that they could be linked to the form field they pertain to.
