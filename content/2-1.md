...
* Explain how headings are read aloud by text-to-speech technologies.
* Explain how heading serve as visual landmark for people with low vision and cognitive disabilities.
* Code headings using the HTML elements `h1` through `h6`, as well as `role="heading'` and `level="1-6" ARIA attributes when standard headings cannot be used.
...
* Explain the purpose and scope of `header`, `nav`, `main`, `section`, `article`, `aside`, and `footer` elements.
