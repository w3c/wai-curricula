...
* write markup for sections of content using the HTML elements `section` or `article`
* write markup for paragraphs using the HTML element `p`
* write markup for lists using the HTML element `ul`, `ol`, or `dl`
* write markup for list items using the HTML elements `li`, `dt`, and `dd`
* write markup for inline or block quotes using the HTML elements `q` or `blockquote` respectively
...
* Define description lists as a grouping of several terms and descriptions. Description lists are marked up with the `dl` element, terms are marked up with the `dt` element, and descriptions are marked up with the `dd` element. Examples of how to code description lists are provided in the WAI Tutorials on [Description Lists](https://www.w3.org/WAI/tutorials/page-structure/content/#description-lists).
