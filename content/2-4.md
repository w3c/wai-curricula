...
* Demonstrate use of the HTML attribute `scope` and its values `row` and `column` to code the direction of the headers. Examples on how to use the HTML attribute `scope` are provided in the WAI tutorials, [Tables with two headers](https://www.w3.org/WAI/tutorials/tabular-information/two-headers/).
* Show examples of headers that span multiple rows or columns. Explain how to code them using the HTML elements `col`, `colgroup`, `thead` and `tbody`, as well as the attributes `colspan`, `rowspan`, and `rowgroup`. Examples on how to code headers that span several rows or columns are provided in the WAI tutorials, [Tables with irregular headers](https://www.w3.org/WAI/tutorials/tabular-information/irregular/).
* Show examples of uses of the `header` attribute to associate more than two headers that relate to the same cell. Explain that when this attribute is used, each of the header cells needs to have a unique id reference. The ids of the corresponding header cells need to be separated by spaces in the value of the `headers` attribute. Examples on how to use the `headers` attribute are provided in the WAI tutorials, [Tables with multi-level headers](https://www.w3.org/WAI/tutorials/tabular-information/multi-level/).
...
* list complimentary requirements for authors to write table descriptions
