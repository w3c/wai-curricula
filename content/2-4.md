...
* Research &mdash; Students research screen readers' documentation to find the keystrokes allowing to move through the different table cells.
...
* Practice &mdash; Students are presented with a table and are asked to break it down into several nested lists. Assess how students communicate the underlying structure of the table despite its visual appearance.
...
* Practice &mdash; Students are presented with a table and are asked to provide a short text to describe its purpose. Assess how students use the `caption` element to add such texts.
* Students are presented with a complex table and are asked to provide extended details about its contents and overall structure. Assess how students use the described additional methods to provide that information.
...
* Introduce the `scope` attribute and its values (`row` and `colummn`) to indicate the direction of the headers.
* Introduce the `headers` attribute of a data cel to indicate irregular headers. Explain that when this attribute is used, each of the header cells need to have a unique id reference. The values of the id references of the header cells we want to associate need to be separated by spaces in the value of the `headers` attribute.
...
* Practice &mdash: Students are presented with a table that has no obvious headers. Then they are asked to figure them out and decide if the table could be considered simple or complex. Assess how students distinguish between simple and complex tables.
* Practice &mdash; Students are presented with a simple table, and are asked to identify its header cells and provide them with the corresponding `th` elements. Assess how students use the `th` elements in a given table.
* Practice &mdash; Students are presented with a table with several column or row headres, and are asked to code them appropriately. Assess how students use the `th` and `scope` elements.
