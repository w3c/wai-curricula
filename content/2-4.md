...
* Applied expertise in teaching:
  ...
  * HTML `table`, [`tr`](https://html.spec.whatwg.org/multipage/tables.html#the-tr-element), [`th`](https://html.spec.whatwg.org/multipage/tables.html#the-th-element), and `[td`](https://html.spec.whatwg.org/multipage/tables.html#the-td-element) elements.
  ...
  * HTML [`scope`](https://html.spec.whatwg.org/multipage/tables.html#attr-th-scope) and `[headers`](https://html.spec.whatwg.org/multipage/tables.html#attr-tdth-headers) attributes.
...
Explain strategies that people use to navigate simple tables, such as moving around adjacent cells. Mention that there might be different ways in which information in a table can be presented depending on user requirements.
...
* Mark up headers that expand multiple rows with the `tbody` element.
...
* Show examples of headers that expand multiple rows or columns, and how to code them using the elements `colgroup` and `body`. For reference on how to code headers that expand several rows or columns, see the WAI website tutorials [Irregular headers](https://www.w3.org/WAI/tutorials/tables/irregular/).
* Show examples of uses of the `header` attribute to denote more than two headers that relate to the same cell. Explain that when this attribute is used, each of the header cells need to have a unique id reference. The ids of the corresponding header cells need to be separated by spaces in the value of the `headers` attribute. For reference on how to use the `headers` attribute, see the WAI website tutorials, [Tables with multi-level headers](https://www.w3.org/WAI/tutorials/tables/multi-level/).

#### Ideas to Assess Knowledge for Topic

Optional ideas to support assessment.

* Short answer questions &mdash; Students are asked how table headers that expand several rows or columns are coded in HTML. Assess students' knowledge of the HTML elements to structure tables.
...
* Show examples of the `caption` element. Explain that it can be used to provide descriptions for the purpose of the table. Mention that it needs to be the first child of the `table` element and that its contents are visible for all users by default. For examples about how to use the caption` element, see technique [H39: Using caption elements to associate data table captions with data tables](https://www.w3.org/WAI/WCAG21/Techniques/html/H39).
...
* Explain that the `summary` attribute was designed to provide screen reader users with detailed information about the structure of complex tables. Emphasize that it is not part of the HTML5 specification, thus its use is not advisable. For examples about how to use the `summary` attribute, see technique [H73: Using the summary attribute of the table element](https://www.w3.org/WAI/WCAG21/Techniques/html/H73
...
* Portfolio &mdash; Students add tables to the website they are building. Assess how students code table headers and table cells, as well as how they provide a table caption and additional descriptions for tables when needed.
...
and that provides a better user experience for everyone
