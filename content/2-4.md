...
* Applied expertise in teaching:
  ...
  * HTML attributes [`scope`](https://html.spec.whatwg.org/multipage/tables.html#attr-th-scope) and [`headers`](https://html.spec.whatwg.org/#attr-tdth-headers)attributes
...
Explain use of HTML elements to mark up tables, table headers
...
Explain use of additional techniques to convey structure and layout for complex tables. For example, the HTML elements `colgroup` and the attributes `scope` and `headers`.
...
* mark up headers that expand several columns using the HTML attribute `colgroup`
* mark up headers that expand multiple rows using the HTML element `tbody`
...
* Explain the complex tables have several column or row header that expand through several columns or rows. Indicate that assistive technologies may not interpret the direction and scope of such headers, so it is necessary to explicitly associate header cells and data cells.
* Demonstrate use of the HTML attribute `scope` and its values `row` and `column` to indicate the direction of the headers. For reference on how to use the HTML attribute `scope`, see the WAI website tutorials, [Tables with two headers](https://www.w3.org/WAI/tutorials/tabular-information/two-headers/).
* Show examples of headers that expand multiple rows or columns and how to code them using the HTML elements `colgroup` and `tbody`. For reference on how to code headers that expand several rows or columns, see the WAI website tutorials, [Irregular headers](https://www.w3.org/WAI/tutorials/tabular-information/irregular/).
...
* Practice &mdash; Students are presented with a table containing headers that expand several columns or rows and are asked to code them. Assess how students use the HTML elements `th`, `col`, `tbody`, and the attribute `scope` to indicate the scope of the table header and convey their direction.
...
Explain use of HTML elements and WAI-ARIA attributes to provide information about the purpose of a table. Mention that providing descripttions for a table is a shared responsibility among different team members: content authors, designers, and developers.
