...
* Research &mdash; Students research screen readers' documentation to find the keystrokes allowing to move through the different table cells. Assess students understanding of screen readers' documentantion.
...
* Introduce the `headers` attribute of a data cel to handle irregular headers. Explain that when this attribute is used, each of the header cells need to have a unique id reference. The ids of the corresponding header cellsneed to be separated by spaces in the value of the `headers` attribute.
