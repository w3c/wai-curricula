...
* mark up custom widgets with appropriate roles, accessible names, and descriptions when necessary
* mark up states and properties of custom widgets to reflect their current state in an accessible way
...
* mark up custom widgets using native HTML interactive elements, such as controls and links, with the necessary styling
* mark up custom widgets using additional WAI-ARIA attributes when native HTML interactive elements are not sufficient
...
* mark up accessible names for custom widgets by using HTML elements and attributes such as `label`, `title`, and WAI-ARIA attributes such as `aria-label` and `aria-labelledby`
* mark up descriptions for custom widgets by adjusting label texts and by using the WAI-ARIA attribute `aria-describedby` when necessary
