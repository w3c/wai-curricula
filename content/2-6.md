...
* Use scripts to add or remove focus to an element based on user interaction.
...
* Introduce the value `-1` for the `tabindex` attribute. Explain that it serves to mark an element so that it can be focusable when it becomes available or visible via scripts.
...
* Explain that event listeners allow for several ways of interaction. For example, keyboard, mouse, and tactile gestures. Emphasize that it is not enought with makin an element focusable but it is also necessary to take care of all possible interaction mechanisms of our users.
