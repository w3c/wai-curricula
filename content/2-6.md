...
* explain when to use WAI-ARIA roles, states, and properties to code accessibility properties of  custom widgets
...
* explain how WAI-ARIA allows text-to-speech technologies to render accessibility properties of complex widgets
...
### Topic: When to Use WAI-ARIA

{% include excol.html type="middle" %}

Present some scenarios where WAI-Aria allows to develop custom components. For example, expanding native semantics of host languages such as HTML. Emphasize that WAI-ARIA is only recommended when there is no alternative in the set of standard HTML elements.

#### Learning Outcomes for Topic

Students should be able to:

* define the scope of the WAI-ARIA specification
* explain how WAI-ARIA overrides accessibility properties of the elements where it is applied
* code custom widgetts using WAI-ARIA roles and properties when HTML elements cannot be used

#### Teaching Ideas for Topic

Optional ideas to teach the learning outcomes.

* Show examples of when WAI-ARIA can be used to complement the semantics of the host language where it is applied. Explain how the WAI-Aria roles, states, and properties override the accessibility properties of web content and applications. Emphasize that WAI-ARIA does not substitute existing semantics of host languages where it is used, but it complements and expands them. For reference on the WAI-ARIA specification, see [WAI-ARIA overview](https://www.w3.org/WAI/standards-guidelines/aria/).
...
* Practical &mdash; Students are presented with different widgets and are asked to decide if WAI-AIRA needs to be used. For those where WAI-ARIA needs to be used,  they are asked which WAI-ARIA roles and properties they should have. Assess students' understanding of the purpose and scope of the WAI-ARIA specification.
...
Explain how to add or remove focus to widgets that are not focusable by default. Emphasize that focus management is essential for keyboard, text-to-speech technologies to interact with non-standard widgets.
...
Describe other states of a widget such as `selected`, `pressed`, `expanded`, or `collapsed`. Explain how to make them distinguishable from the `focus` state.
...
* list requirements for designers to mark up states of a widget, such as distinctive styles or icons
