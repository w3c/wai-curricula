...
* write markup for custom widgets with appropriate roles, accessible names, and descriptions when necessary
* write markup for states and properties of custom widgets to reflect their current state in an accessible way
...
* write markup for accessible names for custom widgets by using HTML elements and attributes such as `label`, `title`, and WAI-ARIA attributes such as `aria-label` and `aria-labelledby`
* write markup for descriptions for custom widgets by adjusting label texts and by using the WAI-ARIA attribute `aria-describedby` when necessary
