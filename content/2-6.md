...
* Short Answer Questions &mdash; Students are asked what are the values of the `tabindex` element and what it means. Assess students' knowledge of the `tabindex` element and its values.
...
Describe additional states of a widget such as `selected`, `pressed`, `draggable`, etc. Explain how to make them distinguishable from the `focused` state.

#### Learning Outcomes for Topic

Students should be able to: 

* Code the expanded or collapsed state using the `aria-expanded` attribute.
* Code the checked state of an object using the `aria-checked` attribute.
* Code the selected state using styles and the `aria-selected` attribute.
...
* Show examples of non-standard checkboxes. Mention the use of `aria-checked` to communicate the state of the checkbox. For reference on how to use the `aria-checked` attribute, see the WAI-ARIA authoring practices, [Checkbox](https://www.w3.org/TR/wai-aria-practices/#check box).
...
* Mark up additions, removals, or all changes in the node to be announced using the `aria-relevant`
 attribute.
