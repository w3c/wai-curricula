...
#### Topic: The Accessibility Tree

{% include excol.html type="middle" %}

Introduce the accessibility tree. Explain its main properties, roles, and states.

#### Learning Outcomes

Students should be able to:

* Identify accessibility name and accessibility description properties.
* Identify other accessibility roles, states, and properties.
* Map standard elements's implicit semantics with exposed accessibility properties.
...
Explain how to add or remove focus to elements that are not focusable by default. Remember focus indicators for keyboard-only users. Emphasize that focus management is essential for keyboard and screen reader users to interact with non-standard widgets.
...
### Topic: Conveying Other States 

{% include excol.html type="middle" %}

Introduce other states such as `selected`, `pressed`, `draggable`, etc. Explain how to make them distinguishable from the `focused` state for keyboard-only users.
