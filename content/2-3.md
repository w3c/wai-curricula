...
* Understand how screen reader users access text alternatives for images using specific keystrokes or functions.
* Understand how people using speech input software can focus and interact with functional images by voicing their text alternative.
* Understand how mobile users can access alternative texts when images are turned off due to data restrictions.
* Unddrstand the role of alternative texts to help better image indexing and ranking of images.
...
* Code SVG graphics with the `alt` attribute, or the `aria-label`, or `aria-labelledby` properties to provide text alternatives.
* Style texts with decorative effect with CSS Transforms and CSS Fonts technologies instead of using images of text.
* Convey mathematical expressions  with MathML language instead of using images of text.
* Markup long descriptions for images with the following techniques:
  * `longdesc` attribute.
