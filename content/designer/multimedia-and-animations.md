...
* Invite students to research about the general flash and red flash ratios. Emphasize that all interface components must flash only up to three times in one second or be below the general flash and red flash thresholds. Any flashing interferes with the user's ability to use the whole interface. For references on how to design interfaces that flash below the general flash and red flash thresholds or that flash up to three times in a second, see [Understanding Success Criterion 2.3.1: Three Flashes or Below Threshold](https://www.w3.org/WAI/WCAG21/Understanding/three-flashes-or-below-threshold).
