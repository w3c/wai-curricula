...
* Show examples of interfaces where information is provided through vision only, such as those with icons to convey when a store is open or closed. Explain that some people cannot rely on visual means to obtain information, so text (or alternatives) are needed for them to understand the information provided. 
