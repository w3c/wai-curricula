...
* Markup blocks of navigation with the `nav` element.
* Group items in a menu with the `ul` or `ol` elements.
* Label menu items and navigational elements with text, the `aria-label`, or the `aria-labelledby` elements.
