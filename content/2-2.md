...
* Specify correct use of HTML elements to organize, identify, and label navigational menus.

## Learning Outcomes for Module

Students should be able to:

* IDentify navigational menus using headings or the `nav`, element.
* Convey the structure of navigational menus using `ul` and `li`, elements.
* Code menu and submenu items using the `a` element and `href` attribute.
* Label menu and submenu items.
...
* Identify navigational menus using headings and/or the `nav` HTML element.
* Group items of the same navigational menu using the HTML elements `ul`, `ol`, and `li`.
* Code menu items using the `a` HTML element and the `href` HTML attribute.
* Label menu items using link text and/or the `aria-label` or `aria-labelledby` attributes.
...
Explain use of HTML elements and WAI-ARIA attributes to indicate the presence of submenu items, for example, the `aria-haspopup` attribute. Explain how to convey visible or hidden states of submenu items using WAI ARIA or text to make them operable by all users.
