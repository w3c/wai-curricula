...
* Define site maps as Web pages that provides links to different sections of the site. Explain that they can be ordered alphabetically, by topic, or by category within the website's structure. For reference on how to provide a site map, see technique [G63: Providing a site map](G63: Providing a site map).
...
* Introduce the HTML `ul`, `ol`, and `li` elements. Explain that they serve to structure and organize list and list items. For examples on how to use these elements within a menu structure, see WAI website tutorials [Menu Representation](https://www.w3.org/WAI/tutorials/menus/structure/#menu-representation).
* Introduce the `ul` and `li` elements. Explain that they are used to mark up lists and list items, which help users understand how many items the menu contains. Emphasize that screen reader users can quickly navigate between lists and list items by using specific keystrokes. Explain that this is a WCAG requirement under success criterion [4.1.2 Name, Role, Value](https://www.w3.org/WAI/WCAG21/quickref/#name-role-value).
...
* Introduce the `aria-haspopup` attribute. Explain that it serves to denote the presence of submenu items. Indicate that this attribute should be present in the original HTML markup. For reference on how to use this attribute, see the WAI website tutorials, [Indicate submenus](https://www.w3.org/WAI/tutorials/menus/flyout/#indicate-submenus).
