...
* Demonstrate accessible and inaccessible modals or dialogs. Emphasize how assistive technologies can access only the relevant elements of such dialogs when they have been coded appropriately. Explain that `aria-hidden="true"` should be applied to the contents that are not part of the modal as long as the modal is displayed, and that `aria-hidden="false"` should be given to the background elements when the modal disappears.
