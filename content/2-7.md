...
* code coherent overall structure and relationships across rich application components that are perceived by people with disabilities
* code visible and hidden states in applications so that they are communicated to assistive technologies
* code the necessary reading and keyboard focus order to navigate between within rich applications using multiple input devices
...
* code mechanisms to switch updates or notifications on or off to avoid conflicts with assistive technologies presenting several notifications at the same time
...
* code document object model additions or removals to match the visual appearance of the application

### Teaching Ideas

Optional ideas to teach the learning outcomes:

* Demonstrate the use of voice commands, keystrokes, or gestures provided by speech and mainstream technologies to query for the application title. Explain that this title is conveyed through the use of the HTML element `title`. 
* Demonstrate the use of voice commands, keystrokes, or gestures provided by speech and mainstream technologies to query the first heading of level 1 for an application. Explain that this is crucial for mobile assistive technologies, where the contents of the HTML element `title` may not be visible due to the smaller screen. Emphasize that at least one heading level 1 should be present in the application's HTML markup, and should be updated accordingly to match any major view change.
* Demonstratre changing application views using an assistive technology. Emphasize that document object model layers should be added and removed depending on the application view, so that assistive technologies only access the ones that are visible
...
* code focus placement when application view changes or errors that reqiure user attention occur
...
* Refer back to previous modules and explain that the tab sequence needs to be preserved so that keyboard users can navigate between the different application components, such as different toolbars or menus that the application may contain. Emphasize that HTML standard elements are recommended regardless of the nature of the application in use, as they inherit all the necessary semantic properties that make them usable with the keyboard. If a non-standard element needs to be used, explain that the `tabindex="0"` value needs to be added so that it can receive keyboard focus.
* Demonstrate use of common shortcut keys for rich applications such as to create a new message in a web-based email client or to move to the menu bar in a word processor. Explain that these shortcut keys should avoid any conflict with key combinations of both browsers and assistive technologies. Emphasize that application functionality should be always guaranteed without the use of such keyboard shortcuts, as they may sometimes be difficult to remember for some users or impossible to use due to their specific configurations.
* Refer back to previous modules and explain focus placement techniques, such as moving focus to the first error message of a form or to the default button in a dialog. Emphasize that depending on the context of the application oand user interaction, focus movements may be required to place focus in the most revelant place according tyo the view or the user action.
...
Demonstrate the use of mechanisms to prioritize updates and notifications based on user interaction or relevance.

#### Learning Outcomes for Topic

Students should be able to:

* code notifications based on user interaction, such as warnings about possible data loss
