...
Demonstrate the use of assistive technologies to change application views and to move across application widgets. Provide examples of different mechanisms to update DOM nodes containing view titles, headings, or regions to match the state of the application at a given time.
...
* ensure elements that are no longer in use are cleaned-up from the DOM
...
* Refer back to [Topic Keyboard and Focus Management](/curricula/custom-widgets/#topic-keyboard-and-focus-management) and explain that the tab sequence needs to be preserved so that keyboard users can navigate between the different application widgets, such as different applications toolbars or menus. Emphasize that HTML standard elements are recommended regardless of the nature of the application in use, as they inherit all the necessary semantic properties that make them usable with the keyboard. If a non-standard element needs to be used, explain that `tabindex="0"` needs to be added so that it can receive keyboard focus.
...
* code messages to inform about waiting times for new data loads or view selections using assertive live regions
